...
public class D15 {
    ...
    public void testNumberGame() {
        assertEquals(436, playGame(new int[] {0,3,6}, 2020));
        assertEquals(1428, playGame(new int[] {2,0,6,12,1,3}, 2020));
        assertEquals(175594, playGame(new int[] {0,3,6}, 30000000));
        assertEquals(3718541, playGame(new int[] {2,0,6,12,1,3}, 30000000));
    }

    private int playGame(int[] startingNumbers, int iterations) {
        Map<Integer, Integer> lastSpoken = new HashMap<>();
        for(int i=0; i<startingNumbers.length; i++)
            lastSpoken.put(startingNumbers[i], i+1);

        int lastNumber = startingNumbers[startingNumbers.length-1];
        for(int i=startingNumbers.length; i<iterations; i++) {
            if(lastSpoken.containsKey(lastNumber)) {
                int indx = lastSpoken.get(lastNumber);
                lastSpoken.put(lastNumber, i);
                lastNumber = i-indx;
            } else {
                lastSpoken.put(lastNumber, i);
                lastNumber = 0;
