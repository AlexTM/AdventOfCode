...
public class D10 {
    ...
    private long combinations(List<Long> joltage, Long value, Long target, Map<Long, Long> cache) {
        ...
        long ret = 0;
        int i=0;
        while(i < joltage.size()) {
            if(joltage.get(i) <= value+3) {
                ret += combinations(
                        joltage.subList(i+1, joltage.size()),
                        joltage.get(i), target, cache);
            }
            i++;
        }
