 ...
 * The card transforms the subject number of the door's public key according to the card's loop size. The result is
 * the encryption key.
 * The door transforms the subject number of the card's public key according to the door's loop size. The result is
 * the same encryption key as the card calculated.
 ...
 * out that the card's loop size must be 8, because transforming the initial subject number of 7 with a loop size of
 * 8 produces 5764801.
 *
 ...
 * encryption key. Transforming the subject number of 17807724 (the door's public key) with a loop size of 8 (the
 * card's loop size) produces the encryption key, 14897079. (Transforming the subject number of 5764801 (the card's
 * public key) with a loop size of 11 (the door's loop size) produces the same encryption key: 14897079.)
 *
 ...
 * stars you owe the resort before you leave. Noticing that you look concerned, the reindeer wanders over to you; you
 * see that it's carrying a small pouch.
 *
...
public class D25 {
    ...
    private long getPublicKey(long subjectNumber, long loopSize) {
        long value = 1;
    ...
    public void testCard() {
        assertEquals(8, getLoopSize(7, 5764801));
        assertEquals(11, getLoopSize(7, 17807724));
        assertEquals(14897079, getEncKey(7, 5764801, 17807724));
        assertEquals(14897079, getEncKey(7, 17807724, 5764801));
    }

    @Test
    public void crackCard() {
        assertEquals(42668, getEncKey(7, 2084668, 3704642));
    }
}
