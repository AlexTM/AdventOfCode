 ...
 * deck of space cards! You'd like to play a game of Combat, and there's even an opponent available: a small crab
 * that climbed aboard your raft before you left.
 *
 ...
 * card, so both cards move to the bottom of player 1's deck such that 9 is above 5. In total, it takes 29 rounds
 ...
 * of the card multiplied by 1, the second-from-the-bottom card is worth the value of the card multiplied by 2, and
 * so on. With 10 cards, the top card is worth the value on the card multiplied by 10. In this example, the winning
 * player's score is:
 *
 *    3 * 10
 * +  2 *  9
 ...
 * other games are not considered. (This prevents infinite games of Recursive Combat, which everyone agrees is a bad
 * idea.)
 ...
 * winner's card is above the other card). Note that the winner's card might be the lower-valued of the two cards if
 * they won the round due to winning a sub-game. If collecting cards by winning the round causes a player to have all
 ...
 * on the card they just dealt, the winner of the round is determined by recursing into a sub-game of Recursive
 * Combat. (For example, if player 1 draws the 3 card, and player 2 draws the 7 card, this would occur if player 1
 * has at least 3 cards left and player 2 has at least 7 cards left, not counting the 3 and 7 cards that were drawn.)
 *
 ...
 * decks to form the sub-game. (For example, if player 1 drew the 3 card, their deck in the sub-game would be copies
