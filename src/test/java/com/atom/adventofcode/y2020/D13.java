...
public class D13 {
    ...
    private List<Long> getBusIds(String stringBusIds){
        return Arrays.stream(stringBusIds.split(","))
                .map(s -> s.equals("x") ? null : Long.parseLong(s))
                .collect(Collectors.toList());
    }

    @Test
    public void testBusTimeTable() {
        Long earliestTime = 939L;
        String stringBusIds = "7,13,x,x,59,x,31,19";

        List<Long> busIds = getBusIds(stringBusIds);
        assertEquals(295, earliestBus(earliestTime, busIds));

        earliestTime = 1005595L;
        stringBusIds = "41,x,x,x,x,x,x,x,x,x,x,x,x,x,x,x,x,x,x,x,x,x,x,x,x,x,x,x,x,x,x,x,x,x,x,37,x,x,x,x,x,557,x,29,x,x,x,x,x,x,x,x,x,x,13,x,x,x,17,x,x,x,x,x,23,x,x,x,x,x,x,x,419,x,x,x,x,x,x,x,x,x,x,x,x,x,x,x,x,x,x,19";

        busIds = getBusIds(stringBusIds);
        System.out.println(earliestBus(earliestTime, busIds));
        assertEquals(2095, earliestBus(earliestTime, busIds));
    }

    private Long earliestBus(Long earliestTime, List<Long> busIds) {
        Map<Long, Long> waitTimes = new HashMap<>();
        for(Long i : busIds) {
            if(i != null) {
                Long startPoint = earliestTime / i;
                waitTimes.put(i, i * (startPoint + 1) - earliestTime);
            }
        }
        Map.Entry<Long, Long> e = Collections.min(waitTimes.entrySet(), Map.Entry.comparingByValue());
        return e.getValue()*e.getKey();
    }


    @Test
    public void testBusContest() {
        assertEquals(160, contest(getBusIds("5,7,9")));
        assertEquals(3417, contest(getBusIds("17,x,13,19")));
        assertEquals(754018, contest(getBusIds("67,7,59,61")));
        assertEquals(779210, contest(getBusIds("67,x,7,59,61")));
        assertEquals(1261476, contest(getBusIds("67,7,x,59,61")));
        assertEquals(1202161486, contest(getBusIds("1789,37,47,1889")));
        assertEquals(1068781, contest(getBusIds("7,13,x,x,59,x,31,19")));

        String stringBusIds = "41,x,x,x,x,x,x,x,x,x,x,x,x,x,x,x,x,x,x,x,x,x,x,x,x,x,x,x,x,x,x,x,x,x,x,37,x,x,x,x,x,557,x,29,x,x,x,x,x,x,x,x,x,x,13,x,x,x,17,x,x,x,x,x,23,x,x,x,x,x,x,x,419,x,x,x,x,x,x,x,x,x,x,x,x,x,x,x,x,x,x,19";
        System.out.println("Result: "+ contest(getBusIds(stringBusIds)));
        assertEquals(598411311431841L, contest(getBusIds(stringBusIds)));
    }


    private Long contest(List<Long> busIds) {
        // Build position map
        Map<Long, Integer> position = new HashMap<>();
        for(int i=0; i<busIds.size(); i++) {
            if(busIds.get(i) != null)
                position.put(busIds.get(i), i);
        }

        // Filter non null
        List<Long> filteredList = busIds.stream()
        ...
        return res.period - res.offset;
    }

    record Column(Long period, Long offset){}

    private Column calculateRepetitionPeriod(Column firstColumn, Column secondColumn) {
        Long period = firstColumn.period;
        Long offsetDiff = secondColumn.offset - firstColumn.offset;
        while(true) {
            if((period+offsetDiff) % secondColumn.period == 0) {
                Long newPeriod = firstColumn.period * secondColumn.period;
                Long newOffset = newPeriod - period + firstColumn.offset;
                return new Column(newPeriod, newOffset);
            }
            period += firstColumn.period;
        }
    }
