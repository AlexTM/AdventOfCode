...
public class D17 {
    ...
    private Set<Pos> generateShape(int p, long delta_x, long delta_y) {
    ...
    private Long getTowerSize(Set<Pos> state) {
        return state.stream().map(p -> p.y).reduce(0L, Long::min);
    }

    private boolean inLimits(int currentShape, long delta_x, long delta_y, int width, Set<Pos> state) {
    ...
    record Info(long height, long count){}
...
//        return        testInput.length()*shapes.size();
    ...
    public void testFallingRocks2() {
        ...
        long size = Math.abs(getTowerSize(state));
        total += size;
