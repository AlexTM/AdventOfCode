 ...
 * December 25th. Although the Elves assure you that the grove has plenty of fruit, you decide to grab any
...
public class D1 {
    ...
    private LoadState loadData(String filename) {
        return FileReader.readFileForObject(filename, new LoadState(), (line, state) -> {
            if(line.isEmpty()) {
                state.calories.add(state.count);
                state.count = 0;
                return state;
            }
            state.count += Integer.parseInt(line);
            return state;
        });
    };

    @Test
    public void testFindElf() {
        List<Integer> caloriesList = loadData("src/test/resources/2022/D1.txt").calories;

        Integer max = caloriesList
                .stream()
                .mapToInt(v -> v)
                .max().orElseThrow(NoSuchElementException::new);
        assertEquals(69693, max);

        caloriesList.sort(Collections.reverseOrder());
        assertEquals(200945, caloriesList.get(0)+caloriesList.get(1)+caloriesList.get(2));
    }
}
