...
public class Day7P1 {
    ...
    class Amp {
        ...
        public Amp(String instructions) {
            codes = IntComputer2.loadInstructions(instructions);
        }

        public void run(int[] inputs, int[] outputs) {
            IntComputer2.compute(codes, inputs, outputs);
        }
    ...
    public void testAmps() {
        String inst = "3,15,3,16,1002,16,10,16,1,16,15,15,4,15,99,0,0";

        List<Amp> ampChain = new ArrayList<>();

        for(int i=0; i<5; i++)
            ampChain.add(new Amp(inst));

        int[] phase = new int[]{4,3,2,1,0};
        int[] outputs = new int[1];
        int result = 0;
        for(int i=0; i<5; i++) {
            ampChain.get(i).run(new int[]{phase[i], result}, outputs);
