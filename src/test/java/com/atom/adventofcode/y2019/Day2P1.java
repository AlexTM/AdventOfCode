...
public class Day2P1 {
    ...
    public void testIntcodeBasic() {
        int[] codes = IntComputerOld.loadInstructions("1,10,20,30,99", 40);
        codes[10] = 1;
        codes[20] = 2;
        IntComputerOld.compute(codes);
        assertEquals(3, codes[30]);
    }

    @Test
    public void testIntcode() {
        int[] codesA = IntComputerOld.loadInstructions("1,9,10,3,2,3,11,0,99,30,40,50");
        IntComputerOld.compute(codesA);
        assertEquals(3500, codesA[0]);

        int[] codesB = IntComputerOld.loadInstructions("1,0,0,0,99");
        IntComputerOld.compute(codesB);
        assertEquals(2, codesB[0]);

        int[] codesC = IntComputerOld.loadInstructions("2,3,0,3,99");
        IntComputerOld.compute(codesC);
        assertEquals(6, codesC[3]);

        int[] codesD = IntComputerOld.loadInstructions("2,4,4,5,99,0");
        IntComputerOld.compute(codesD);
        assertEquals(9801, codesD[5]);

        int[] codesE = IntComputerOld.loadInstructions("1,1,1,4,99,5,6,0,99");
        IntComputerOld.compute(codesE);
