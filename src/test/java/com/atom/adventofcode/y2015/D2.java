...
public class D2 {
    ...
    record Parcel(int l, int w, int h){};

    private List<Parcel> readParcels(String filename) {
        return FileReader.readFileObjectList(filename, this::getParcel);
    }

    private Parcel getParcel(String inp) {
        String[] spit = inp.split("x");
        return new Parcel(Integer.parseInt(spit[0]),Integer.parseInt(spit[1]),Integer.parseInt(spit[2]));
    }

    private int calculatePaper(Parcel p) {
        int s1 = p.l*p.w;
        int extra = s1;
        int s2 = p.w*p.h;
    ...
    @Test
    public void testWrappingPaper() {
        assertEquals(58, calculatePaper(getParcel("2x3x4")));
        assertEquals(1588178,
                readParcels("src/test/resources/2015/D2.txt")
                        .stream().mapToInt(this::calculatePaper).sum());
    }

    private int calculateRibbon(Parcel p) {
        int len = 2*(p.h+p.l);
    ...
    public void testRibbon() {
        assertEquals(34, calculateRibbon(getParcel("2x3x4")));
