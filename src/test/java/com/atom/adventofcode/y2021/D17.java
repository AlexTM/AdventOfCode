...
public class D17 {
    ...
    record TargetArea(int x1, int y1, int x2, int y2){};
    record Pair(int x, int y){};

    // brute force it.  Could add bounds based on target area
    List<Pair> generatePossibleSolutions(TargetArea ta) {
    ...
    public int fireY(Pair v, TargetArea ta) {
        Pair pos = new Pair(0,0);
        int maxHeight = 0;
        while(true) {
            pos = new Pair(pos.x + v.x, pos.y + v.y);
            ...
            v = new Pair(Math.max(0, v.x-1), v.y-1);
        }
    }

    public int getHighestPoint(TargetArea ta) {
        return generatePossibleSolutions(ta).stream().map(p -> fireY(p, ta)).max(Integer::compareTo).get();
    }
    ...
    // brute force it.  Could add bounds based on target area
    public List<Pair> generatePossibleSolutionsBothAxis(TargetArea ta) {
        List<Pair> ps = new ArrayList<>();
    ...
    public int fire(Pair v, TargetArea ta) {
        Pair pos = new Pair(0,0);
        int maxHeight = 0;
        while(true) {
            pos = new Pair(pos.x + v.x, pos.y + v.y);
            maxHeight = Math.max(maxHeight, pos.y);
            if(pos.x > ta.x2 || pos.y < ta.y2) {
                return -1;
            }
            if(pos.x >= ta.x1 && pos.x <= ta.x2 &&
                    pos.y <= ta.y1 && pos.y >= ta.y2) {
                return maxHeight;
            }
    ...
    public int solutionSize(TargetArea ta) {
        return (int)generatePossibleSolutionsBothAxis(ta).stream().filter(p -> fire(p, ta) >= 0).count();
    }
