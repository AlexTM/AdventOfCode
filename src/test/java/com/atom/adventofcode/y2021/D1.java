 ...
 * A (199, 200, 208); their sum is 199 + 200 + 208 = 607. The second window is marked B (200, 208, 210); its sum is
 * 618. The sum of measurements in the second window is larger than the sum of the first, so this first comparison
 * increased.
 *
    ...
    private long countDepthIncreasesWithSlidingWindow(List<Integer> depths) {
        ...
        return depths.stream().filter(d -> {
            ...
            if(state.queue.size() == 4) {
                state.sum -= state.queue.poll();
                res = currentDepth < state.sum;
            }
            return res;
        }).count();
    }
