...
public class D11 {
    ...
    static private Map<Pos, Integer> loadData(String data) {
        char[] inps = data.toCharArray();
        Map<Pos, Integer> map = new HashMap<>();
        for (int i = 0; i < 10; i++) {
            for (int j = 0; j < 10; j++) {
                map.put(new Pos(i, j), inps[i + (j * 10)] - '0');
            }
        }
        return map;
    }

    public Set<Pos> getAdjacent(Pos p) {
    ...
    public long step(Map<Pos, Integer> data) {
        Deque<Pos> queue = new LinkedList<>(data.keySet());
        Set<Pos> flashed = new HashSet<>();
        while (!queue.isEmpty()) {
            Pos current = queue.pop();
            if (flashed.contains(current))
                continue;

            int v = data.get(current) + 1;
    ...
    public long countFlashesInLoops(Map<Pos, Integer> data, int loops) {
        long flashes = 0;
