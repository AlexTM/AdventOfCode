...
public class D7 {
    ...
    public int calculateCost(int[] input, int target, Function<Integer, Integer> costFunction) {
        int cost = 0;
        for(int i : input)
            cost += costFunction.apply(Math.abs(target - i));
        return cost;
    }

    public int findBest(int[] testInput, Function<Integer, Integer> costFunction) {
        int max = Arrays.stream(testInput).max().getAsInt();
        int min = Arrays.stream(testInput).min().getAsInt();
        int bestCost = Integer.MAX_VALUE;

        // Must be a more efficient way than this
        for(int i=min; i<=max; i++) {
            bestCost = Math.min(bestCost, calculateCost(testInput, i, costFunction));
        }
